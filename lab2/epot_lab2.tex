...
Os valores medidos no laboratório para cada uma dessas grandezas foi:

\todo{por valores do lab e fazer comparaçoes}

\paragraph{Característica de comando do conversor}

Para se estudar convenientemente o funcionamento do circuito face a variações do ângulo de disparo $\alpha$, registaram-se os valores da tensão na carga em função deste tanto no caso experimental como teórico fazendo uso das expressões na secção anterior. Os resultados obtidos podem ser observados em \textcolor{red}{tabela e gráfico}. 

\todo{inserir tabela, gráfico e comentar} 
